...
BezierPatch3.prototype.B3 = function(i,t)
{
	var t1 ;
	switch ( i ) {
...
BezierPatch3.prototype.dB3 = function (i, t)
{
	var t1 ;
	switch ( i ) {
		case 0:
...
BezierPatch3.prototype.draw = function () {
...
BezierPatch3.prototype.GetVertex = function (u, v)
{

	vd = {} ;
	var dPu = vec3(0.0,0.0,0.0) ;
	var dPv = vec3(0.0,0.0,0.0) ;
	var p = vec3(0.0,0.0,0.0) ;
	
	for( var i = 0 ; i < 4 ; i++ )
	{
		var bi = this.B3(i,u) ;
		var dbi = this.dB3(i,u) ;
		for( var j = 0 ; j < 4 ; j++ )
		{
			p = add(p, scalev(bi*this.B3(j,v), this.cpoints[i][j])) ;
			dPu = add(dPu, scalev(dbi*this.B3(j,v), this.cpoints[i][j])) ;
			dPv = add(dPv, scalev(bi*this.dB3(j,v), this.cpoints[i][j])) ;
		}
	}
	...
	vd.colour = vec4(0.0,0.5,0.0,1.0) ;
...
BezierPatch3.prototype.SetData = function(prog){
